...
# credentials resolve through the ambient aws chain; fill this
# dict only when the runner needs explicit keys
dynamodb_config = {}
...
def parse_revisions_from_file(file_contents: bytes) -> tuple[str | None, str | None]:
    revision = None
    down_revision = None
...
def yield_alembic_ini_paths() -> Generator[str]:
...
def yield_alembic_migrations_directories() -> Generator[str]:
...
def get_alembic_revision_graph(migrations_dir: str) -> Tuple[dict, str, dict]:
    ...
    for dirpath, dirnames, filenames in os.walk(migrations_dir):
        for filename in filenames:
            ...
            if down_revision is None:
                if origin_revision:
                    raise RuntimeError(f"found multiple origin revisions: {origin_revision}, {revision}")
    ...
    if not origin_revision:
        raise RuntimeError("no origin revision found")
...
def insert_node(graph: dict, revision_map: dict, revision: str, children: tuple, parents: tuple, filepath: str) -> None:
    ...
    for child in children:
        if child not in graph:
            raise RuntimeError(f"internal error: child revision {child} not found in graph")
        graph[child]["parents"] += (revision,)

    for parent_rev in parents:
        if parent_rev not in graph:
            raise RuntimeError(f"internal error: parent revision {parent_rev} not found in graph")
...
def alembic_name(path: str) -> str:
    # return alembic folder name (i.e. `alembic_billing`) from path
    return re.search(r"(alembic_\w+)", path).group(1)


def git_path(path: str) -> str:
    # determine git path from file system path; the bot always runs
    # from the repository root, where the two coincide
    return path
...
def send_slack_notifications() -> None:
    ...
    if git_head := get_current_git_head():
        pretext_parts.append("Commit: <{}|{}>".format(
            f"https://github.com/{REPOSITORY}/commit/{git_head}",
            git_head[:10],
        ))
